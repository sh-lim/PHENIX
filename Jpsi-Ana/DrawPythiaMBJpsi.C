void DrawPythiaMBJpsi(){
	...
	infile[0] = new TFile("outfile_pythiaCharm_pp13TeV.root","read");
	...
	for (int iset=0; iset<nset; iset++){
...

		{
			...
			TH1D *htmp = (TH1D*)gPad->DrawFrame(0,1e-5,60-iset*35,1.2*h1d_svx[iset][0]->GetMaximum());
			...
			leg->AddEntry(hfvtxz_mult2[0][0],"J/#psi, 1.2<|y|<2.2","p");
			leg->AddEntry("",Form("mean=%4.2f",hmult2[0][0]->GetMean()),"");
			leg->AddEntry(hfvtxz_mult2[1][0],"#psi(2S), 1.2<|y|<2.2","p");
			leg->AddEntry("",Form("mean=%4.2f",hmult2[1][0]->GetMean()),"");
			leg->Draw();
			*/
		}

		{
			...
			TH1D *htmp = (TH1D*)gPad->DrawFrame(0,0.7,60-iset*35,1.3);
